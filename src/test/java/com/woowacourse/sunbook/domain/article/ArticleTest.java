...
class ArticleTest {
    ...
    void 게시글_정상_생성_이미지_비디오_존재() {
        assertDoesNotThrow(() -> new Article(new ArticleFeature(CONTENTS, IMAGE_URL, VIDEO_URL)));
    }

    @Test
    void 게시글_정상_생성_내용없고_이미지_존재_비디오_존재() {
        assertDoesNotThrow(() -> new Article(new ArticleFeature(EMPTY, IMAGE_URL, VIDEO_URL)));
    }

    @Test
    void 게시글_정상_생성_이미지_없고_비디오_존재() {
        assertDoesNotThrow(() -> new Article(new ArticleFeature(CONTENTS, EMPTY, VIDEO_URL)));
    }

    @Test
    void 게시글_정상_생성_이미지_존재_비디오_없음() {
        assertDoesNotThrow(() -> new Article(new ArticleFeature(CONTENTS, IMAGE_URL, EMPTY)));
    }

    @Test
    void 게시글_정상_생성_내용만_있음() {
        assertDoesNotThrow(() -> new Article(new ArticleFeature(CONTENTS, EMPTY, EMPTY)));
    }

    @Test
    void 게시글_정상_생성_이미지만_있음() {
        assertDoesNotThrow(() -> new Article(new ArticleFeature(EMPTY, IMAGE_URL, EMPTY)));
    }

    @Test
    void 게시글_정상_생성_비디오만_있음() {
        assertDoesNotThrow(() -> new Article(new ArticleFeature(EMPTY, EMPTY, VIDEO_URL)));
    }

    @Test
    void 게시글_생성_오류_아무것도_없음() {
        assertThrows(IllegalArgumentException.class, () -> new Article(new ArticleFeature(EMPTY, EMPTY, EMPTY)));
    }

    @Test
    void 게시글_정상_수정() {
        Article article = new Article(new ArticleFeature(CONTENTS, IMAGE_URL, VIDEO_URL));
