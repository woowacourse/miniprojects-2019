...
// 로그인한 유저 정보 가져오기
request.get('/users/loggedin/', (status, data) => {
    loggedInUser = data;

    // 이 아티클을 좋아요하는 유저들 가져오기
    request.get(`/articles/${articleId}/likes`, (status, data) => {
        likedUsers = data;

        // 유저들 수가 곧 좋아요 수
        likeNum.innerText = `${likedUsers.length} 명이 좋아합니다.`;

        // 로그인한 유저가 좋아요하는 유저들에 포함되는지 여부
        console.log(likedUsers.includes(loggedInUser));
